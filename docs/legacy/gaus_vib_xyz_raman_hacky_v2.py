...
# vib-xyz.py v. 0.0.0
#
# Cartesian decomposition of vibrational spectra.
#
# Contributor: Chloe Groome (UC Irvine)
#
# URL: http://raman-sc.github.io <REPLACE WITH OUR GITHUB URL>
# If you utilize this code for a published work, please site:
# <PAPER CITATION>
...
def parse_gaus(gaus_fh):
    ...
    pol = [[[None for _ in range(3)] for _ in range(3)] for _ in range(len(eigvals))]
    ...
    #start the line search from the top of the file
    gaus_fh.seek(0)
    # read all the files from the file into the 'lines' list
    lines = gaus_fh.readlines()
    # iterate through groups of five lines
    for line1, line2, line3, line4, line5 in zip(lines, lines[1:], lines[2:], lines[3:], lines[4:]):
        if "Polarizability derivatives wrt mode" in line1:
            c = line3.split()[1:4]; print(c)
            pol[j][0] = [float(x.replace('D', 'e')) for x in line3.split()[1:4]]
            pol[j][1] = [float(x.replace('D', 'e')) for x in line4.split()[1:4]]
            pol[j][2] = [float(x.replace('D', 'e')) for x in line5.split()[1:4]]
            j += 1
...
def main():    
    ...
    #i altered this code from this source:
    #we should definitely credit them, but also, i did add quite a bit to theirs and expanded it
    ...
    #raman calculation and output. all formulas are correct, but code may need to be spruced up

    bohr2ang = 0.52917724924
    
    output_fh = open('gaus_raman.csv', 'w')
    output_fh.write("# mode,freq,alpha,beta2,activity,activity_x,activity_y,activity_z\n") # freq in cm-1
    for i in range(len(eigvals)):
        eigval = float(eigvals[i])
        eigvec = eigvecs[i]
        norm = norms[i]
        ...
        print("")
        print("! %4i  freq: %10.5f  alpha: %10.7f  beta2: %10.7f  activity: %10.7f  activity_x: %10.7f  activity_y: %10.7f  activity_z: %10.7f "
